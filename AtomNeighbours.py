...
class AtomNeighbours:
    ...
    def neigbourhood_matrix(self, batch) -> torch.Tensor: 
        """
        ...
        if not isinstance(batch.pos, torch.Tensor):
            raise TypeError("batch.pos must be a torch.Tensor")
        if not isinstance(batch.batch, torch.Tensor):
        ...
        adj_matrix = (batch.batch.unsqueeze(1) == batch.batch.unsqueeze(0)).int()
        ...
        x_ij = batch.pos[idx_i, 0] - batch.pos[idx_j, 0]  # x-coordinate difference (x_i - x_j)
        y_ij = batch.pos[idx_i, 1] - batch.pos[idx_j, 1]  # y-coordinate difference (y_i - y_j)
        z_ij = batch.pos[idx_i, 2] - batch.pos[idx_j, 2]  # z-coordinate difference (z_i - z_j)
